...
$plus= Html::a('<i class="fas fa-plus"></i>', 
			['paesistica/nuova'], ['title'=>'Aggiungi Istanza', 'class'=>'btn btn-success']);
...
<!-- Modal -->
<div class="modal" id="myAnnoModal" tabindex="-1" role="dialog" >
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body" >
        <form action="/index.php?r=paesistica/elencoautorizzazioni" id ="formemail" method="post">
          <div class="form-group" id="deto">
              <label for="eto" class="col-form-label">anno:</label>
              <input type="text" class="form-control" id="selanno">
          </div>
            <div>
                <button type="submit" class="btn btn-primary" id="annoButton">OK</button>
            </div>
        </form>
      </div>
    </div>
...
$pdfHeader='';
$pdfFooter='';
$title='Tabella Pratiche Paesistiche';
...
//        'template' => '{soggetti}{allegati}{atti}', //{procedura}',
//        'buttons' => [
...
//                    $label = $icon . ' ' . $title;
//                    $url = Url::toRoute(['paesistica/soggetti','idpratica'=>$model->idpaesistica]);
//                    $options['tabindex'] = '-1';
...
//                    $label = $icon. ' ' . $title;
//                    $url = Url::toRoute(['paesistica/allegati','idpratica'=>$model->idpaesistica]);
//                    $options['tabindex'] = '-1';
//                    return '<li>' . Html::a($label, $url, $options) . '</li>' . PHP_EOL;
//                },   
...
//                    $label = $icon. ' ' . $title;
//                    $url = Url::toRoute(['paesistica/toword','idpratica'=>$model->idpaesistica]);
//                    $options['tabindex'] = '-1';
//                    return '<li>' . Html::a($label, $url, $options) . '</li>' . PHP_EOL;
//                },  
...
//    ],                    
    [
        ...
        'buttons'=>['atti' => function ($url,$model) {
                  return Html::a('<span class="fas fa-edit"></span>', Url::toRoute(['paesistica/atti','idpratica'=>$model->idpaesistica]),
                      [  
                         'title' => 'Composizione Atti',
...
//            ]
        ...
        ['class'=>'kartik\grid\DataColumn',
         ...
         'label'=>'Data Autorizzazione', 
          'value' => function($model) {
                   if (!empty($model->DataAutorizzazione)) {
         ...
         'filterType' => GridView::FILTER_DATE_RANGE,
         'filterWidgetOptions' => [
                'startAttribute' => 'data_inizio_autorizzazione', //Attribute of start time
                'endAttribute' => 'data_fine_autorizzazione',   //The attributes of the end time
                'convertFormat'=>true, // Importantly, true uses the local - > format time format to convert PHP time format to js time format.
                'pluginOptions' => [
                    'format' => 'yyyy-mm-dd',//Date format
                    'locale'=>['format' => 'd-m-Y'], //php formatting time
...
//        },
        'filter' => Arrayhelper::map(TitoliPaesistica::find()->asArray()->all(), 'idtitoli_paesistica', 'descrizione'),
...
//        'format'=>'raw'
    ...
    'gridOptions'=>[
        ...
        'panel'=>[
		'heading'=>'<h3 class="panel-title"><b>ELENCO PRATICHE PAESISTICHE<b></h3>',
	...
	'toolbar' =>  [
			...
			['paesistica/elencoautorizzazioni'], ['title'=>'Elenco Autorizzazioni per Anno', 'class'=>'btn btn-primary'])
            ],
            ['content'=>Html::tag('div','',['style' => ['margin-left'=>'5px']])],
            ['content'=>Html::a('<i class="fas fa-th-list"></i>', 
			['paesistica/elencopratiche'], ['title'=>'Elenco Numero Pratiche per Anno', 'class'=>'btn btn-primary'])
            ],
    ...
    'options'=>['id'=>'dynagrid-35'] // a unique identifier is important
]);
