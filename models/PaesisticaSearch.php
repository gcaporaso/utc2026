 ...
 */
class PaesisticaSearch extends Paesistica
{
...
//}
    ...
    public $data_inizio_autorizzazione; //start time
    public $data_fine_autorizzazione; //End time
    public $richiedente;
    public $tipo;
    public $statoPratica;
    //public $residuo;
...
//        ];
        ...
        return [
            ...
            [['NumeroProtocollo','DataProtocollo','DescrizioneIntervento','TipoCatasto','tipo','idtipo','statoPratica',
                'idcommittente','IndirizzoImmobile', 'NumeroAutorizzazione','DataAutorizzazione','richiedente',
                'Indennita','IndPagataresiduo'], 'safe'],
            [['data_inizio_protocollo','data_fine_protocollo','data_inizio_autorizzazione','data_fine_autorizzazione','Compatibilia',
                'CatastoFoglio','CatastoParticella','CatastoSub', 'InviatoSoprintendenza','InviatoRegione'],'safe'],
...
//                'Sanatoria','Tipologia','TipoRichiesta','committente_id2','committente_id3'], 'safe'],
    ...
    public function search($params)
    {
		...
		$query = Paesistica::find()
                        ->indexBy('idpaesistica'); //->joinWith(['committente1'=> function($query) { $query->from(['committente1' => 'Committente']); }]); //->all();
     ...
     $dataProvider->setSort([
        'attributes' => [
            ...
            'NumeroAutorizzazione'=>[
                 'desc' => ['NumeroAutorizzazione' => SORT_DESC],
                 'asc' => ['NumeroAutorizzazione' => SORT_ASC],   
                 'default' => SORT_ASC
            ],
            'DataAutorizzazione'=>[
                 'desc' => ['DataAutorizzazione' => SORT_DESC],
                 'asc' => ['DataAutorizzazione' => SORT_ASC],   
                 'default' => SORT_ASC
            ],
        ]
    ]);
...
//        }
        ...
        //    'TipoRichiesta'=>$this->TipoRichiesta,
            ...
            'Compatibilita'=>$this->Compatibilita,
        ...
        if ($this->statoPratica) {
            $query->andFilterWhere(['=', 'StatoPratica', $this->statoPratica]);
        }
        ...
        if ($this->data_inizio_autorizzazione && $this->data_fine_autorizzazione) {
            //$null = new \yii\db\Expression('null');
            $data_inizio_titolo =  date("Y-m-d", strtotime($this->data_inizio_autorizzazione));
            $data_fine_titolo   = date("Y-m-d", strtotime($this->data_fine_autorizzazione));
            $query->andWhere(['between', 'DataTitolo', $data_inizio_autorizzazione, $data_fine_autorizzazione]);
            $query->andWhere(['not', ['DataAutorizzazione'=>null]]);
        }
        
        // Ricerca pratiche per tipo di richiesta
        if ($this->tipo) {
            $query->andFilterWhere(['=', 'idtipo', $this->tipo]);
        }
